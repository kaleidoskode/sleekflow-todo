...
`backend/.env.example` to `backend/.env` and uncomment the 5433 lines — the file explains exactly
when to do this.
...
pip install -e ".[dev]"
cp .env.example .env                    # adjust the port if you used DB_PORT=5433
docker compose up -d db                 # Postgres must be reachable on the port in .env
...
`todo_test` database (created in the quick start above) and a `backend/.env` pointing at the
running Postgres.
