# Engineering Notes — SleekFlow TODO

Long-form companion to the [decision log](decision-log.md). The log is kept to the
one-to-two pages the brief asked for and states each decision; this file keeps the
reasoning, the measurements and the failures behind them.

Everything here is the working record — including three defects found by measuring
rather than by testing, which are written up as found rather than quietly fixed:

- [the default sort was never index-served](#2-architectural-decisions-and-trade-offs)
  — a nullable `due_date` forced a `coalesce()` sort key that no index matched
- **the sentinel was a bound parameter**, so the index matched only when
  PostgreSQL happened to choose a custom plan — 0.9 ms one direction, 4.8 ms the other
- **cycle detection was exponential in graph width** — 42 nodes took 2 seconds, on
  the hot path of every dependency add

Timings and query plans live in [performance.md](performance.md); the component
...
- **A frontend test suite** — the UI is deliberately thin over a typed API client; the complexity
  worth testing lives in backend domain logic, which is where the 182 tests are. The UI is verified
  by the live demo.
